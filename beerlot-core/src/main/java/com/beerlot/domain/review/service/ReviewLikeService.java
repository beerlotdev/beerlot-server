...
public class ReviewLikeService {
    ...
    public List<Long> getReviewLikesByMember(String oauthId) {
        Member member = memberService.findMemberByOauthId(oauthId);

        List<ReviewLike> reviewLikes = reviewLikeRepository.findByMember_Id(member.getId());
        return reviewLikes.stream().map(reviewLike -> reviewLike.getReview().getId()).toList();
    }
